...
test.describe("Ana sayfa @public", () => {
  test("başlık ve ana CTA görünür", async ({ page }) => {
    await page.goto("/");
    await expect(page.getByRole("heading", { level: 1 })).toContainText("Öğretmen bul, ders al, soru sor");
    await expect(page.getByRole("link", { name: "Öğrenci olarak başla", exact: true })).toBeVisible();
    await expect(page.getByRole("link", { name: "Öğretmenleri keşfet" })).toBeVisible();
    await expect(page.getByRole("heading", { name: "BenimÖğretmenim’de kim ne yapar?" })).toBeVisible();
    ...
    await expect(page.getByText("LGS için öğretmen seçme")).toBeVisible();
    await expect(page.getByText("Veli için haftalık takip")).toBeVisible();
  });

  test("hızlı öğretmen arama hunisi query ile listeye gider", async ({ page }) => {
    await page.goto("/");
    await page.getByLabel("Ders, sınav veya konu ara").fill("LGS Matematik");
